...
module.exports = function (context) {
    function setupEnvironmentSigningConfig() {
        ...
        if (
            appConfig &&
            appConfig["signing-dev-team"] &&
            appConfig["signing-dev-team"]["signing"]
        ) {
            ...
            Object.keys(buildConfigs).forEach((configKey) => {
                ...
                appExtensions.forEach((extension) => {
                    if (
                        buildConfig.buildSettings &&
                        buildConfig.buildSettings.PRODUCT_NAME === `"${extension}"`
                    ) {
                        buildConfig.buildSettings.DEVELOPMENT_TEAM = signing;
                    }
                });
    ...
    function editPlistProjectToIncludeFaceIDDescription() {
        try {
            ...
            if (fs.existsSync(projectInfoPlist)) {
                const existingPlistContent = fs.readFileSync(
                    projectInfoPlist,
                    "utf8"
                );
            ...
            if (appConfig && appConfig["ns-face-id-usage-description"]) {
                plistObject.NSFaceIDUsageDescription =
                    appConfig["ns-face-id-usage-description"];
            } else {
                plistObject.NSFaceIDUsageDescription =
        ...
        try {
            ...
            if (!fileContent.includes(authenticateUserMethod)) {
                if (viewDidLoadRegex.test(fileContent)) {
                    ...
                    console.log(
                        `Method ${authenticateUserMethod} inserted inside viewDidLoad.`
                    );
                } else {
                    ...
                    console.log(
                        "viewDidLoad method not found, it was created above @end."
                    );
                }
            } else {
            ...
            if (biometricImport && !fileContent.includes(biometricImport)) {
                ...
                console.log(
                    `Import ${biometricImport} inserted at the beginning of the file.`
                );
            }

            try {
                const hash =
                    "//@b5d72bb4eb6154384459a114f7019c2d==//@ don't remove it";
    ...
    try {
        ...
        const appConfigFile = path.join(projectRoot, "config-build.json");
        const iosPath = path.join(projectRoot, "platforms", "ios");
        const config = utils.getConfigParser(
            context,
            path.join(projectRoot, "config.xml")
        );
        const projectName = config.name();

        const projectInfoPlist = path.join(
            iosPath,
            projectName,
            `${projectName}-Info.plist`
        );
        const hp2ClientInfoPlist = path.join(
            iosPath,
            "Extensions",
            "HP2ClientExtension",
            `Info.plist`
