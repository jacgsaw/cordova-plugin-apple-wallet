...
module.exports = (context) => {
  ...
  logger.addLogDaySection(`Add Swuft support script`)
    ...
    getPlatformVersionsFromFileSystem(context, projectRoot).then((platformVersions) => {
      ...
      for (configName in buildConfigs) {
        if (!COMMENT_KEY.test(configName)) {
          ...
          if (
            xcodeProject.getBuildProperty("SWIFT_OBJC_BRIDGING_HEADER", buildConfig.name) !== bridgingHeaderProperty
          ) {
            xcodeProject.updateBuildProperty("SWIFT_OBJC_BRIDGING_HEADER", bridgingHeaderProperty, buildConfig.name)
            logger.append(
              `Update IOS build setting SWIFT_OBJC_BRIDGING_HEADER to:  ${bridgingHeaderProperty} for build configuration ${buildConfig.name}`
            )
          }
        }
      }
      ...
      glob("**/*Bridging-Header*.h", { cwd: pluginsPath }, (error, files) => {
        ...
        headers.forEach((header) => {
          if (header !== bridgingHeader && !~content.indexOf(header)) {
            ...
            logger.append(`Importing ${header} into ${bridgingHeaderPath}`)
          }
        })
        ...
        for (configName in buildConfigs) {
          if (!COMMENT_KEY.test(configName)) {
            buildConfig = buildConfigs[configName]
            if (
              parseFloat(xcodeProject.getBuildProperty("IPHONEOS_DEPLOYMENT_TARGET", buildConfig.name)) <
              parseFloat(IOS_MIN_DEPLOYMENT_TARGET)
            ) {
              ...
              logger.append(
                "Update IOS project deployment target to:",
                IOS_MIN_DEPLOYMENT_TARGET,
            ...
            if (xcodeProject.getBuildProperty("ALWAYS_EMBED_SWIFT_STANDARD_LIBRARIES", buildConfig.name) !== "YES") {
              ...
              logger.append(
                `Update IOS build setting ALWAYS_EMBED_SWIFT_STANDARD_LIBRARIES to: YES for build configuration ${buildConfig.name}`
              )
            }

            if (
              xcodeProject.getBuildProperty("LD_RUNPATH_SEARCH_PATHS", buildConfig.name) !==
              '"@executable_path/Frameworks"'
            ) {
              const newPropRunPathSearch = [
                '"@executable_path/Frameworks"',
                '"$(inherited)"',
                '"@executable_path/../../Frameworks"',
              ]
              xcodeProject.updateBuildProperty("LD_RUNPATH_SEARCH_PATHS", newPropRunPathSearch, buildConfig.name)
              logger.append(
                `Update IOS build setting LD_RUNPATH_SEARCH_PATHS to: ${newPropRunPathSearch}
                \tfor build configuration:  ${buildConfig.name}`
              )
            }
            ...
            if (typeof xcodeProject.getBuildProperty("SWIFT_VERSION", buildConfig.name) === "undefined") {
              if (config.getPreference("UseLegacySwiftLanguageVersion", "ios")) {
                ...
                logger.append(`Use legacy Swift language version ${buildConfig.name}`)
                ...
                logger.append(`Use Swift language version ${swiftVersion}`)
              } else {
                xcodeProject.updateBuildProperty("SWIFT_VERSION", "4.0", buildConfig.name)
                logger.append(`Update SWIFT version to 4.0 ${buildConfig.name}`)
              }
            }
            ...
            if (buildConfig.name === "Debug") {
              if (xcodeProject.getBuildProperty("SWIFT_OPTIMIZATION_LEVEL", buildConfig.name) !== '"-Onone"') {
                ...
                logger.append(
                  `Update IOS build setting SWIFT_OPTIMIZATION_LEVEL to: -Onone for build configuration ${buildConfig.name}`
                )
              }
            }
          }
